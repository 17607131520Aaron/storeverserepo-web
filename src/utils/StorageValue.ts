...
// 定义存储数据的类型
type TStorageValue = string | Record<string, unknown> | null;
 ...
 */
const setStorage = (storage: Storage, key: string, value: TStorageValue): void => {
 ...
 * @returns 存储的值（字符串或对象）
 */
const getStorage = (storage: Storage, key: string): TStorageValue => {
